...
지금 머지 0인 축: `N+1`(보류 2) · `중복 쿼리`(보류 1) · `쓰기만 하는 컬렉션`(대기 2).
`순차 I/O` 는 ghost#29704 가 머지되면서 검증됐다 — 이 축의 예외는 닫혔다.

**보류 2건이 N+1 축의 전부다.** 축 커버리지 예외의 목적은 규칙을 머지로 검증하는 건데,
낸 것이 전부 보류면 검증은 여전히 0이다. 예외를 다시 쓸 수 있는 축으로 본다.
...
**열린 것 10건(보류 6건 빼면 4건).** 판정 난 12건 중 머지 7 · 승인 0 · 닫힘 5.
<!-- /auto:open -->
...
| 저장소 | 중앙 | 평균 | 외부 머지 | 메모 |
|---|---|---|---|---|
| vscode | 0.2일 | 1.5일 | 17/60 | 링크된 이슈 없는 외부 PR 을 봇이 닫는다 — 게이트 0 성격 |
| ghost | 0.2일 | 1.1일 | 28/60 | **#29831 로 나갔다** |
| react | 0.7일 | 4.8일 | 8/60 | 후보의 소비자가 `/* DISABLED */` 주석 안 — PR 보다 이슈 |
| next.js | 0.8일 | 91.9일 | 6/60 | 평균은 이상치 탓 — 외부 머지가 6건뿐이라 문이 좁다 |
| nx | 1.0일 | 4.9일 | 26/60 | 예전에 31.9일로 적어둔 값이 중앙 1.0일로 바뀌었다 |
| storybook | 1.1일 | 7.6일 | 37/60 | **다음 순번.** 외부 머지 비율이 제일 높다 |
| angular | 1.7일 | 9.9일 | 37/60 | 주석이 "쓴다"고 적힌 자리라 설명이 길어진다 |
| astro | 2.0일 | 13.3일 | 39/60 | |
| pnpm | 6.0일 | 8.1일 | 15/60 | |
| rollup | 8.5일 | 9.0일 | 12/60 | |
...
이유는 위 표에 그대로 있다. next.js 는 평균 91.9일인데 중앙은 0.8일이다 — 묵은 몇 건이
평균을 통째로 끌고 간다. 평균을 기준선으로 쓰면 **그 저장소가 실제로 얼마나 빠른지가
아니라 제일 오래 방치된 PR 이 판정을 정한다.** 우리 보드에서도 excalidraw 가 평균
70.1일이라 6일째 PR 이 "여유 있음"으로 보였는데, 중앙 1.1일로 보면 이미 늦은 것이었다.
