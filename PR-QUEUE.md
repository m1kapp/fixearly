...
| tailwindcss | 0.2일 | 63% | `ast.ts:502` 는 keyframe 수(기본 테마 ~10) — 탈락 |
...
**안 냈다 — 규칙 ① 때문이다.** 이 코드가 가장 흔하게 만나는 switch 는 case 10개 미만이고,
거기서는 이득이 0이다. directus#27978 이 닫힌 사유("이 엣지 케이스의 성능 이득은 churn 을
정당화하지 못한다")와 같은 모양이다.

브랜치는 만들어 뒀다(`perf/no-duplicate-case-set`, 테스트 29/29 통과, lint 훅 통과).
꺼낼 조건: **eslint 쪽에 룰 성능 이슈가 열리거나**, 규칙 ① 의 "중앙" 정의를 린트 룰에는
다르게 적용하기로 정할 때. 후자는 규칙을 바꾸는 일이라 여기서 혼자 정하지 않는다.
