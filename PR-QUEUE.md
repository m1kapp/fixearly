...
| [ghost#29831](https://github.com/TryGhost/Ghost/pull/29831) | 쓰기만 하는 컬렉션 | ⚪ 대기 | 오늘 / 평균 1일 |
...
**열린 것 10건(보류 5건 빼면 5건).** 판정 난 12건 중 머지 7 · 승인 0 · 닫힘 5.
<!-- /auto:open -->
...
ghost 를 고른 이유는 회전이 제일 빠르고(중앙 0.3일) 같은 stats 영역에 이미 머지된
...
| 저장소 | 평균 | 중앙 | 외부 머지 | 메모 |
|---|---|---|---|---|
| ghost | 1.1일 | 0.3일 | 28/60 | **#29831 로 나갔다** |
| vscode | 1.5일 | 0.2일 | 17/60 | 링크된 이슈 없는 외부 PR 을 봇이 닫는다 — 게이트 0 성격 |
| react | 4.8일 | 0.7일 | 8/60 | 후보의 소비자가 `/* DISABLED */` 주석 안 — PR 보다 이슈 |
| nx | 4.9일 | 1.0일 | 26/60 | 예전에 31.9일로 적어둔 값이 4.9일로 바뀌었다 |
| storybook | 7.6일 | 1.1일 | 37/60 | **다음 순번.** 외부 머지 비율이 제일 높다 |
| pnpm | 8.1일 | 6.0일 | 15/60 | |
| rollup | 9.0일 | 8.5일 | 12/60 | |
| angular | 9.9일 | 1.7일 | 37/60 | 주석이 "쓴다"고 적힌 자리라 설명이 길어진다 |
| astro | 13.3일 | 2.0일 | 39/60 | |
| next.js | 91.9일 | 0.8일 | 6/60 | 평균은 이상치 탓 — 외부 머지가 6건뿐이라 문이 좁다 |

**평균과 중앙이 이만큼 갈린다.** next.js 는 평균 91.9일인데 중앙은 0.8일이다 — 오래
묵은 몇 건이 평균을 통째로 끌고 간다. 지금 랜딩 카드와 보류 판정은 평균을 쓰는데,
그 기준이면 이상치 하나에 판정이 흔들린다. **중앙값으로 바꾸는 게 맞다** (별건).
