...
| `throw-path-runs-once` | O(n²) 배열 조회 | 반복대상과 같은 배열을 스캔해 '같이 자람'으로 떴지만, 그 스캔은 throw 하는 블록 안의 오류 메시지 조립이었다 — 실패할 때 한 번 돈다 | jest `jest-config/src/index.ts:283·285` (`ensureNoDuplicateConfigs`) — 그 저장소 `[같이자람]` 2건이 전부 이것. webpack `ExportsInfo.js:707` 도 호출자 쪽에서 같은 이유였다(파일 밖이라 이 가드로는 못 잡는다) | 수신자에서 루프까지 올라가며 `throw` 문을 가진 블록이 있으면 `throw-path` 컷 + coGrows 해제 | `quadratic-co-growth.ts` |
