...
| `promise-all-batching` | 루프 안 DB/HTTP (N+1) | `Promise.all(items.map(async …))` 는 병렬 배칭인데 순차 N+1 로 봤다 | 코퍼스 스캔 | 루프 안에서 **직접 await** 하는 호출만 센다 | 없음 — `n-plus-one.ts` 에 배칭 모양을 넣어 뒀지만, 가드(중첩 함수 경계·`inCombinator`)를 둘 다 제거해도 그 자리가 안 잡힌다. 무엇이 실제로 막고 있는지 못 짚었으므로 **닫힌 것으로 세지 않는다** |
