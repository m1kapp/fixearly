...
const qualityGrade =
  qualityScore >= 88 ? "S" :   // 코퍼스 상위 ~15% — 예외적으로 깨끗
  qualityScore >= 78 ? "A" :   // 상위 ~45%
  qualityScore >= 65 ? "B" :   // 중앙권
  qualityScore >= 45 ? "C" :   // 기본선 — 실제 서비스/앱은 구조 지표상 불리하므로 여기가 바닥
  qualityScore >= 30 ? "D" :   // 드묾 — 구조가 실제로 무너진 경우
  "E";                         // 사실상 방치
