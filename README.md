...
| **O(n²)** | 5 | 행 루프 안에서 바깥 배열 조회. 바깥 배열을 `.find/.some` 으로 훑는 자리 — Map/Set 이면 O(1)이다. **이 도구가 PR 을 가장 많이 만든 축인데 채점에는 없었다**(11건 제출·머지 1·승인 1). 등급을 안 움직이면서 고칠 줄만 주고 있었던 셈이다. 오래 뺐던 이유("n 이 큰지 사람이 봐야 한다")는 세 겹으로 막았다: ① 테스트·프론트 구역, 정적 외곽(const-outer), 루프 안 I/O, n 이 잘린 자리(capped-n)를 제외한 **candidates** 만 센다 ② **3곳 미만 면제** ③ 캡 5 — 판정 난 O(n²) PR 6건 중 4건이 닫혔다(오탐률 ~36%). **10k줄당**으로 잰다: 파일당으로 재면 코드줄과 rho=+0.69 로 규모를 못 지운다. 줄 기준으로 바꾸면 종류 안에서 코드줄 상관이 전부 음수가 된다(툴체인 −0.14·프레임워크 −0.25·앱 −0.38) — 전체 +0.64 는 종류 교란이었다. 유예 1.0/10k줄, 기울기 1.0(캡은 6.0에서). 37곳 중 17곳 감점, 2곳만 만점. |

**상위 10개 평균을 쓰는 이유**: 최악값 1개를 재면 "가장 나쁜 하나만 고치고 끝"이 최적 전략이 된다(실측: 하나 고치면 +0.99점, 그 다음부터 ~0). 상위 10개 평균은 10개를 실제로 줄여야 계속 내려간다.

### 진단 18종 (등급을 건드리지 않는다 · 파일:줄만 짚는다)
...
**복잡도 축과 함수 길이 축은 상당히 겹친다 — 알고 두는 것이다.** 70곳 원지표 상관:
...
- **오픈소스 70개로 보정** — 유예값은 임의 임계가 아니라 코퍼스 중앙값이고, 기울기는 p90에서 캡에 닿게 잡았다. (중앙 81)
  **그 규칙을 스스로 감사한다.** 7개 감점 항 중 6개는 p90 ±5% 안에서 캡에 닿는데, `평균 파일 길이`만 145에서 닿고 있었다
  (p90은 280). 캡을 14→5로 줄이면서 나눗수를 안 고쳐 도달점이 190→145로 끌려 내려간 누락이었고, 70곳 중 29곳이
