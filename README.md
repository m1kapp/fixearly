...
이미 정적 축이 깨끗한 저장소를 계속 뒤질 때는 `--sweep`을 붙인다. 분석을 한 번 더 하지 않고 기존 진단을
**깊게 검증 / 맥락 확인 / 보류**로 접은 뒤, 현재 SHA를 로컬 기준선에 저장한다. 다음 실행부터는 신규·유지·해소만
보여준다. 여기의 0~100은 실제 성능·영향 점수가 아니라, 비싼 실행 검증을 어디부터 할지 정하는 정적 우선순위다.

```bash
npx fixearly --dir=src --sweep --report
```
...
`--deps`는 두 가지를 같이 본다. 하나는 락파일의 (이름, 버전)을 OSV.dev에 물은 **고쳐진 버전이 나온 취약점**,
다른 하나는 **저자가 중단 선언한 직접 의존**(npm registry의 `deprecated`)이다. 뒤엣것은 "신버전에 더 좋은 게
생겼나"의 부분집합이다 — 그 질문의 일반형은 CHANGELOG를 읽어야 알지만, 중단 선언은 저자가 직접 쓴 문장이라
추측이 없고 대개 후속 패키지까지 적혀 있다. 최신도 중단 상태면 "올려도 안 풀린다"고 따로 말한다.
`npm outdated`가 하는 "몇 버전 뒤처졌나"는 넣지 않았다 — 한 저장소에서 263개 중 190개가 뒤처져 있었고,
190줄짜리 목록은 아무도 안 본다.

권고 버전이 **이 프로젝트의 배포 런타임에서 도는지**도 함께 본다. Dockerfile의 `FROM node:20`,
`.nvmrc`, `package.json`의 `engines` 중 가장 낮은 값이 실제 하한이고, 권고 버전의 `engines`가
그보다 높으면 "런타임이 선행"이라고 말한다. CI 러너가 Node 22여도 배포 이미지가 Node 20이면
그냥 올린 상태로 타입체크·CI가 전부 통과하고 배포에서만 터진다 — 실제로 한 번 겪었다.
