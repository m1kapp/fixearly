...
| **O(n²)** | 5 | 행 루프 안에서 바깥 배열 조회. 바깥 배열을 `.find/.some` 으로 훑는 자리 — Map/Set 이면 O(1)이다. **이 도구가 PR 을 가장 많이 만든 축인데 채점에는 없었다**(11건 제출·머지 1·승인 1). 등급을 안 움직이면서 고칠 줄만 주고 있었던 셈이다. 오래 뺐던 이유("n 이 큰지 사람이 봐야 한다")는 세 겹으로 막았다: ① 테스트·프론트 구역, 정적 외곽(const-outer), 루프 안 I/O, n 이 잘린 자리(capped-n)를 제외한 **candidates** 만 센다 ② **3곳 미만 면제** ③ 캡 5 — 판정 난 O(n²) PR 6건 중 4건이 닫혔다(오탐률 ~36%). **10k줄당**으로 잰다: 파일당으로 재면 코드줄과 rho=+0.69 로 규모를 못 지운다. 유예 1.0/10k줄, 기울기 1.0(캡은 6.0에서). **이 기울기는 "캡은 p90에서" 규칙을 의도적으로 벗어난다** — 74곳 p90은 3.95라 캡이 p90의 152% 지점에 있다. 중복 축이 유예를 중앙 위로 올린 것과 같은 이유다: 판정 난 O(n²) PR 6건 중 4건이 닫혔으니, 오탐이 남는 축은 캡뿐 아니라 기울기도 완만해야 한다. 74곳 재검증에서 발동 29곳 · 만점 5곳 · 기존 점수와 rho=−0.20. |
